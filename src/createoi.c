 ...
 *
 *  Contains definitions for all the functions available in COIL.  Many functions are direct
 *  implementations of Open Interface commands (making this a wrapper).  However, others are
 *  provided as additional functionality that is not available in the OI, provided as a
 *  convience to the user.  It is best to read the accompaning documentation for usage info.
 *
...
int startOI (char* serial)
{
	...
	if (0 == fd)
	{
		...
		tcgetattr (fd, &options);				//get config from fd and put into options
		cfmakeraw (&options);					//give raw data path
		cfsetispeed (&options, B57600);			//set baud
		cfsetospeed (&options, B57600);
		tcsetattr (fd, TCSANOW, &options);		//send options back to fd
	}
	
	if (cwrite (fd, cmd, 1) < 0)
	{
		perror ("Could not start OI");
		return -1;
	}
	return 0;
}
...
{
  int shouldStop=0;
  shouldStop = readSensor (SENSOR_BUMPS_AND_WHEEL_DROPS) + readSensor (SENSOR_OVERCURRENT) +
							getCliffs();
  return shouldStop;
}
